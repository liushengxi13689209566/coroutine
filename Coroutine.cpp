...
Coroutine::Coroutine(CoroutineSchedule *sch, CoFun func, int id)
	: func_(func), id_(id), sch_(sch),
	  ...
	  status_(CO_READY), CorStack(nullptr)
{
}
Coroutine::~Coroutine()
{
	CorStack.reset(nullptr);
}
...
void Coroutine::save_stack(void *top)
{
	...
	if (stack_max_size_ < (char *)top - &dummy)
	{
		...
		// CorStack = std::make_shared<char>(stack_max_size_);
		CorStack.reset(new char[stack_max_size_]);
	}
	stack_cur_size_ = (char *)top - &dummy;
	memcpy(reinterpret_cast<void *>(CorStack.get()), &dummy, stack_cur_size_);
}
} // namespace Tattoo
